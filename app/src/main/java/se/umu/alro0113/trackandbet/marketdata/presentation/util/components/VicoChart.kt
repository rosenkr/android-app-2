...
// Formats a long date into a smaller date and returns it as a String
// TODO put in its own file as "DateConverter"
fun convertToMyDate(longDate : String?) : String{
    ...
    // Define the output formatter to get "d MMM" format
    val outputFormatter = DateTimeFormatter.ofPattern("dMMM")

    return dateTime.format(outputFormatter)
}
/*
https://www.youtube.com/watch?v=MY290aW8hMQ uses a texhbutton that when pressed increases counter
a launchedeffect is using this counter to trigger a recomposition if his graph

in my case, I want a recompositon of the graph to happen every time the (intended to be a server in
the future) client makes an api call for the current ticker, which should happen every X seconds
if the market is open for that ticker (so a graph is obviously static during weekends)

Note for future: marketstack has stock data going back 10+ years, in the case of a server,
can repopulate its database with that old data, which the graph should initilly be composed with

example with client making api calls:
if(marketOpen){
    launchedEffect(key1 = timerState.value){
     // add add recent api call most recent value, have to think a bit here..
    }
}

Idea: he clears old data set. I need to SAVE old data set, add 1 more timeframe of data to the set, and then recompose with the newly modified data set
 */
